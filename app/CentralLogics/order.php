...
class OrderLogic
{
    ...
    public static function gen_mis_report($orders) 
    {
        $data = [];
        foreach($orders as $key=>$order) {
            ...
            $data[] = [
                'Sl'=>$key,
                'Order Data' => isset($order->schedule_at)? date('Y-m-d h:i a', strtotime($order->schedule_at)) : date('Y-m-d h:i a', strtotime($order->created_at)),
                'Order Id' => $order->id,
                'Vendor Id' => $order->store?$order->store->vendor_id:translate('messages.store_not_found'),
                'Store Name' => $order->store?$order->store->name : translate('messages.store_not_found'),
                'Store Id' => $order->store_id ,
                'Payment Mode' => $order->payment_method ,
                'Vendor Price' => $order->transaction ? $order->transaction->store_amount - $order->transaction->tax : 0 ,
                'Item Price' => $order->details_sum_price ,
                'Delivery Fees' => $order->delivery_charge,
                'Tax(with GST)' => $order->transaction ? $order->transaction->tax : 0,
                'Total Selling Price' => $order->order_amount,
                'Promo Code' => $order->coupon_code ,
                'Promo Amount' => $order->coupon_discount_amount,
                'Admin Commission' => $order->transaction ? $order->transaction->admin_commission : 0,
                'Customer ID' => $order->user_id ,
                'Customer Name' => $order->customer ?  $order->customer->f_name .''. $order->customer->l_name : translate('messages.customer_not_found'),
                'Customer Mail' => $order->customer ? $order->customer->email : translate('messages.customer_not_found'),
                'Customer Mobile' => $order->customer ? $order->customer->phone : translate('messages.customer_not_found'),
                'Delivery Address' => $order->delivery_address ? json_decode($order->delivery_address, true)['address'] : ' ' ,
                'Delivery boy' => isset($order->delivery_man) ? $order->delivery_man->f_name . ' ' . $order->delivery_man->l_name : translate('messages.not_found'),
                'Order Placed Time' => isset($order->created_at) ? date('Y-m-d h:i a', strtotime($order->created_at)) : date('Y-m-d h:i a', strtotime($order->schedule_at)),
                'Accepted Time' => date('Y-m-d h:i a', strtotime($order->confirmed)),
                'Dispatched Time' => date('Y-m-d h:i a', strtotime($order->handover)),
                'Completed Time' => date('Y-m-d h:i a', strtotime($order->delivered)),
                'Vendor Zone' => ($order->store && $order->store->zone)? $order->store->zone->name :translate('zone_deleted')
            ];
        }
        return $data;
    }
